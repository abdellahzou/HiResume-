...
// Component to handle root redirection based on browser language
const RootRedirect = () => {
  const getBrowserLang = (): Language => {
    // Get language part (e.g. 'fr-CA' -> 'fr')
    const browserLang = navigator.language.split('-')[0];
    const validLangs: Language[] = ['en', 'fr', 'es'];
    // Default to 'en' if the browser language is not supported
    return validLangs.includes(browserLang as Language) ? (browserLang as Language) : 'en';
  };

  return <Navigate to={`/${getBrowserLang()}`} replace />;
};
