...
export const Builder: React.FC<BuilderProps> = ({ t }) => {
  ...
  // Logic: Auto-scroll stepper and page reset
  useEffect(() => {
  ...
  // Logic: Handle PDF Export with "Fit to Page" scaling
  const handlePdfExport = () => {
    const element = document.getElementById('resume-preview');
    if (!element) return;

    // 1. Set filename for print job
    ...
    // 2. Calculate Scaling for A4 (1123px height at 96 DPI)
    ...
    if (contentHeight > a4HeightPx) {
       // Scale to fit 100% of height with a small margin
       scale = (a4HeightPx - 10) / contentHeight;
       scale = Math.max(scale, 0.55); // Prevent scaling to unreadable sizes
    }

    // 3. Apply scale and trigger print
    ...
    <div className="flex flex-col w-full min-h-[calc(100vh-80px)]">
      ...
      <div className="sticky top-16 z-30 bg-white/95 backdrop-blur-sm border-b border-gray-100 shadow-sm no-print">
        <div className="max-w-7xl mx-auto">
          <div ref={stepperRef} className="flex items-center gap-2 overflow-x-auto px-4 py-3 scrollbar-hide snap-x">
            {steps.map((step) => {
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 no-print">
                <div className="mb-6">
                  <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                    <Download size={20} className="text-blue-600"/> {t.nav.preview}
                  </h3>
